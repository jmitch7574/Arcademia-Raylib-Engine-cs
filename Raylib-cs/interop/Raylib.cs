...
public static unsafe partial class Raylib
{
    ...
    static Raylib()
    {
        // Custom raylib web assembly loader, thanks to zed dev on the raylib discord
        NativeLibrary.SetDllImportResolver(Assembly.GetExecutingAssembly(), (libraryName, assembly, searchPath) =>
        {
            // Add these lines for WebAssembly handling
            if (libraryName == "raylib" && OperatingSystem.IsBrowser())
            {
                return NativeLibrary.Load("raylib");
            }
        
            return ResolveDllImport(libraryName, assembly, searchPath);
        });
    }
