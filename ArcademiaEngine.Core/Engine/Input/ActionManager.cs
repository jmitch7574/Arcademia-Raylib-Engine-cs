...
public class ActionMap
{
    ...
    private static bool CheckActionGlobal(string actionName, bool includeNonPlayers, Func<KeyboardKey, bool> keyCheck, Func<int, GamepadButton, bool> gamepadCheck)
    {
        ...
        var activeKeyboardSlots = InputManager.Players.Where(p => p.Input.IsKeyboard).Select(p => p.Input.InputIdx).ToHashSet();
        var activeControllerSlots = InputManager.Players.Where(p => !p.Input.IsKeyboard).Select(p => p.Input.InputIdx).ToHashSet();
